...
def _spinner_svg() -> Any:
    """Return the SVG spinner for the button's loading state."""
    return html.svg(  # type: ignore[operator]
        aria_hidden="true",
        role="status",
        class_="inline w-4 h-4 me-3 animate-spin",
        viewBox="0 0 100 101",
        fill="none",
        xmlns="http://www.w3.org/2000/svg",
    )(
        html.path(  # type: ignore[attr-defined]
...
class Button:
    ...
    def htmy(self, context: Context) -> Component:
        ...
        button_content: Any = (
            [_spinner_svg(), html.span(self.label)] if self.loading else self.label
        )
